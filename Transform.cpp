...
//
//-----------------------------------------------------------------------------
void Transform::rfft(double* x, long N, unsigned int forward)
{
    static int first = 1 ;
    ...
    wpr = (double) (-2.*pow( sin( 0.5*theta ), 2. )) ;
...
std::vector<double> Transform::rfft(std::vector<double> audio, unsigned int forward) {
    ...
    rfft(x, length/2, forward);
    audio.assign(x, x + length);
    delete[] x;
    return audio;
}
//-----------------------------------------------------------------------------
...
//
//   cfft replaces double array x containing NC complex values (2*NC double
...
//
//-----------------------------------------------------------------------------
void Transform::cfft(double* x, long NC, unsigned int forward)
{
...
//-----------------------------------------------------------------------------
// name: bit_reverse()
// desc: bitreverse places double array x containing N/2 complex values
//       into bit-reversed order
//-----------------------------------------------------------------------------
void Transform::bit_reverse(double * x, long N)
{
    double rtemp, itemp ;
    long i, j, m ;
